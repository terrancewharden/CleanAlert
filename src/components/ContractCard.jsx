...
export default function ContractCard({ contract, onExpress, isNew }) {
  return (
    <div style={{
      background: "#ffffff",
      ...
      {/* Title */}
      <div style={{ color:"#0a1628", fontWeight:800, fontSize:17, marginBottom:"0.75rem", lineHeight:1.3, paddingRight: contract._seed ? 64 : 0 }}>
        {contract.title}
      </div>

      {/* Tags */}
      <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:"0.85rem" }}>
        <span style={{ background:"#e0f7ff", color:"#006d8f", fontSize:12, padding:"4px 12px", borderRadius:20, fontWeight:600 }}>{contract.service_type}</span>
        ...
        {contract.sq_footage && <span style={{ background:"#f0fdf4", color:"#166534", fontSize:12, padding:"4px 12px", borderRadius:20, fontWeight:600 }}>{contract.sq_footage}</span>}
        {contract.duration_months && <span style={{ background:"#fffbeb", color:"#92400e", fontSize:12, padding:"4px 12px", borderRadius:20, fontWeight:600 }}>📅 {contract.duration_months}mo contract</span>}
      </div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10, borderTop:"1px solid #f0f4f8", paddingTop:"1rem" }}>
        <div>
          <div style={{ color:"#0a1628", fontSize:12, fontWeight:600, marginBottom:2, opacity:0.5 }}>MONTHLY BUDGET</div>
          <div style={{ color:"#007a4d", fontWeight:800, fontSize:20 }}>{contract.budget || "TBD"}</div>
        </div>
        <button
          onClick={() => onExpress(contract)}
          style={{
            background:"#00d4ff", color:"#0a1628", border:"none", borderRadius:12,
            padding:"0.7rem 1.6rem", fontWeight:800, fontSize:15, cursor:"pointer",
            animation:"pulseRing 2s infinite", letterSpacing:"-0.01em",
          }}
        >
          Express Interest →
        </button>
