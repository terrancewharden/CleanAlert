    ...
    <div style={{ minHeight:"100vh", background:NAVY, fontFamily:"Inter,sans-serif" }}>
      ...
      <nav style={{ borderBottom:`1px solid ${BORDER}`, padding:"1rem 1.5rem", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={()=>nav("/cleaner")} style={{ background:"transparent", color:MUTED, border:`1px solid ${BORDER}`, borderRadius:6, padding:"5px 12px", fontSize:13, cursor:"pointer" }}>Cleaner View</button>
          <button onClick={()=>nav("/buyer")} style={{ background:"transparent", color:MUTED, border:`1px solid ${BORDER}`, borderRadius:6, padding:"5px 12px", fontSize:13, cursor:"pointer" }}>Buyer View</button>
          ...
          <div style={{ background:"#fff", borderRadius:10, padding:"1.25rem", border:"1px solid #e5e7eb", marginBottom:"1.5rem" }}>
            ...
            {activity.map((a,i)=>(
              <div key={i} style={{ display:"flex", gap:12, alignItems:"center", padding:"0.6rem 0", borderBottom: i<activity.length-1?"1px solid #f3f4f6":"none" }}>
                <span style={{ background:a.type==="deal"?"#e0f7ff":"#f5f0ff", color:a.type==="deal"?"#006d8f":"#6d3aaa", fontSize:11, padding:"2px 8px", borderRadius:4, fontWeight:700, letterSpacing:"0.04em" }}>{a.type?.toUpperCase()}</span>
